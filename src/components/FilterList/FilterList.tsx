...
export const FilterList = () => {
  ...
  useEffect(() => {
    ...
    if (sort) {
      filterArray.push({
        value:
          sort === "price:desc" ? "Price : High to Low" : "Price: Low to High",
        key: "sort",
      });
    }
    if (categoryid && categoryList) {
      ...
      filterArray.push({
        value: `Category : ${res?.attributes?.categorytype ?? " "} `,
        key: "categoryid",
      });
    }
    if (minPrice && maxPrice) {
      filterArray.push({
        value: `Budget : ${minPrice} - ${maxPrice}`,
        key: "price",
      });
    }
    if (brandName) {
            ...
            <Stack direction="row" gap={"8px"} className={styles.box}>
              {filterData?.map((data, index) => (
                <Stack direction="row" key={index} className={styles.filter}>
                  <Typography variant="h6" className={styles.filtertext}>
                    {data?.value ?? ""}
                  </Typography>
                  <IconButton
                    sx={{
                    ...
                    onClick={() => handleRemoveFilters(data?.key)}
                  >
